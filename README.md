...
\( \nu = 2.3 \times 10^{-5} \,\mathrm{m^2/s} \), assuming atmospheric pressure.

The solution is evaluated at a streamwise location of **x = 0.3 m** with a free-stream velocity of **U∞ = 4 m/s**.

The similarity domain is truncated at \( \zeta_{\max} = 8 \), which is sufficient for the velocity profile to reach its asymptotic value. A grid of **1000 points** is used, corresponding to a physical wall-normal resolution of approximately **10.5 μm**, ensuring accurate interpolation and numerical integration.
...
\( f'(\zeta_{\max}) \approx 1 \), verifying correct enforcement of the far-field boundary condition.

For Part (b), the wall-normal location where  
\( u_x = B_{\text{given}} \, U_\infty \)  
...
For Part (c), the boundary-layer thickness \( \delta_{99} \) is computed numerically and compared against the empirical estimate  
\( \delta_{99} \approx 5 \sqrt{\nu x / U_\infty} \),  
